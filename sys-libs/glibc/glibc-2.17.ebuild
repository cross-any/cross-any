...
SRC_URI+=" https://dev.gentoo.org/~vapier/dist/${P}-patches-${PATCH_VER}.tar.bz2"
...
src_prepare() {
	rm -f "${WORKDIR}"/patches/6600_mips_librt-mips.patch
	if ! use vanilla ; then
	...
	eapply "${FILESDIR}"/glibc-version-locale-archive.patch
	eapply "${FILESDIR}"/glibc-2.17-To-support-building-glibc-with-GCC-6-configured-with.patch
