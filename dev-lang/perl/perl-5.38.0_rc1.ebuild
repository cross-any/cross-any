...
src_prepare() {
	...
	if use prefix && [[ -e "${CPREFIX}"/usr/include/errno.h ]] ; then
		sed -i "/my..sysroot/s:'':'${CPREFIX}':" ext/Errno/Errno_pm.PL || die
	fi
...
src_configure() {
	...
	if use prefix ; then
		...
		myconf "-Dlibpth=${CPREFIX}/$(get_libdir) ${CPREFIX}/usr/$(get_libdir) ${paths}"
	...
	myconf \
		...
		-Dscriptdir="${CPREFIX}"'/usr/bin' \
		-Dprivlib="${CPREFIX}${PRIV_LIB}" \
		...
		-Dman1dir="${CPREFIX}"/usr/share/man/man1 \
		-Dman3dir="${CPREFIX}"/usr/share/man/man3 \
		-Dsiteman1dir="${CPREFIX}"/usr/local/man/man1 \
		-Dsiteman3dir="${CPREFIX}"/usr/local/man/man3 \
		-Dvendorman1dir="${CPREFIX}"/usr/share/man/man1 \
		-Dvendorman3dir="${CPREFIX}"/usr/share/man/man3 \
		-Dman1ext='1' \
		-Dman3ext='3pm' \
		-Dlibperl="${LIBPERL}" \
		-Dlocincpth="${CPREFIX}"'/usr/include ' \
		-Dglibpth="${CPREFIX}/$(get_libdir) ${CPREFIX}/usr/$(get_libdir)"' ' \
		-Duselargefiles \
		-Dd_semctl_semun \
		-Dcf_by='Gentoo' \
	...
	if tc-is-cross-compiler; then
		./configure \
			...
			-Dotherlibdirs=/usr/$CTARGET_default/usr/lib/perl${PV:0:1}/${PV:0:4} \
			"${myconf[@]}" \
			|| die "Unable to configure"
	else
		sh Configure \
			-des \
			-Dinstallprefix="${CPREFIX}"'/usr' \
